...
int main() {
    ...
    std::cout << "Test with quotes: " << (test_parse_with_quotes() ? "PASS" : "FAIL") << std::endl;
    
    // Debug output for failing tests
    std::cout << "\nDebug output for 'mkfs.so':" << std::endl;
    bool ok;
    auto result = parse_command_line_regex("mkfs.so", ok);
    print_result(result);
    
    std::cout << "\nDebug output for 'mkfs.so;cpiod.so;':" << std::endl;
    result = parse_command_line_regex("mkfs.so;cpiod.so   ;   haproxy.so;", ok);
    print_result(result);
