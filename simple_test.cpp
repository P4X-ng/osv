...
int main() {
    ...
    if (ok && result.size() == 1 && result[0].size() == 2 && 
        ...
        std::cout << "FAIL (size=" << result.size() << ")" << std::endl;
        if (result.size() > 0) {
            std::cout << "  Command 0 size: " << result[0].size() << std::endl;
            for (size_t i = 0; i < result[0].size(); ++i) {
                std::cout << "    [" << i << "] = '" << result[0][i] << "'" << std::endl;
            }
        }
    }
    ...
    if (ok && result.size() == 2 && 
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            std::cout << "  Command " << i << " size: " << result[i].size() << std::endl;
            for (size_t j = 0; j < result[i].size(); ++j) {
                std::cout << "    [" << j << "] = '" << result[i][j] << "'" << std::endl;
            }
        }
    }
