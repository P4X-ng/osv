...
{
        local pattern="$1"
        local count=$(ldconfig -p | grep -E "$pattern" | grep "${MACHINE}" | wc -l)
        ...
        if [[ $count == 0 && $IGNORE_MISSING == false ]]; then
                ...
                ldconfig -p | grep -E "$pattern"
                return -1
        else
                local so_name_path=$(ldconfig -p | grep -E "$pattern" | grep "${MACHINE}")
                so_name=$(echo $so_name_path | awk '{print $1}')
                so_path=$(echo $so_name_path | awk '{print $NF}')
                return 0
        fi
}
...
output_manifest()
{
        ...
        if [[ $dl == "linux" ]]; then
                ...
                lddtree $so_files | grep -v "not found" | grep -v "$so_filter" | grep -v "ld-linux-${MACHINE}" | \
                        grep -Ev 'lib(gcc_s|resolv|c|m|pthread|dl|rt|aio|xenstore|crypt|selinux)\.so([0-9.]+)?' | \
                        sed 's/ =>/:/' | sed 's/^\s*lib/\/usr\/lib\/lib/' | sort | uniq | tee -a $OUTPUT
        else
                lddtree $so_files | grep -v "not found" | grep -v "$so_filter" | grep -v "ld-linux-${MACHINE}" | \
                        grep -Ev 'lib(gcc_s|resolv|c|m|pthread|dl|rt|stdc\+\+|aio|xenstore|crypt|selinux)\.so([0-9.]+)?' | \
                        sed 's/ =>/:/' | sed 's/^\s*lib/\/usr\/lib\/lib/' | sort | uniq | tee -a $OUTPUT
        fi
}
...
detect_elf()
{
        ...
        local elf_filter=$(echo $file_desc | grep -E 'LSB shared object|LSB.*executable' | grep "${MACHINE}" | wc -l)
        if [[ $elf_filter == 1 ]]; then
                local shared_object_filter=$(echo $file_desc | grep -E 'LSB shared object' | wc -l)
